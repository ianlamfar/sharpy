...
import sharpy_control.aero.utils.mapping as mapping
import sharpy_control.utils.cout_utils as cout
import sharpy_control.utils.solver_interface as solver_interface
import sharpy_control.utils.controller_interface as controller_interface
...
import sharpy_control.io.network_interface as network_interface
import sharpy_control.utils.generator_interface as gen_interface
...
class DynamicCoupled(BaseSolver):
    ...
    def time_loop(self, in_queue=None, out_queue=None, finish_event=None, solvers=None):
        ...
        for self.data.ts in range(
            ...
            # print(np.degrees(aero_kstep.control_surface_deflection), np.degrees(structural_kstep.psi[-1, -1, 1]))
            ...
            for k in range(self.settings['fsi_substeps'] + 1):
                ...
                self.aero_solver.update_custom_grid(
                    structural_kstep,
                    aero_kstep)
                # print(k, np.degrees(aero_kstep.control_surface_deflection), np.degrees(structural_kstep.psi[-1, -1, 1]))
                ...
                # run the solver
                # print(np.degrees(aero_kstep.control_surface_deflection), np.degrees(controlled_aero_kstep.control_surface_deflection))
                # print('Before', np.degrees(structural_kstep.psi[-1, -1, 1]), np.degrees(controlled_structural_kstep.psi[-1, -1, 1]))
