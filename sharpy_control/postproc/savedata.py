...
import h5py
import copy
import sharpy_control
import sharpy_control.utils.cout_utils as cout
from sharpy_control.utils.solver_interface import solver, BaseSolver
import sharpy_control.utils.settings as settings_utils
import sharpy_control.utils.h5utils as h5utils
from sharpy_control.presharpy.presharpy import PreSharpy
    ...
    def initialise(self, data, custom_settings=None, caller=None, restart=False):
        ...
        if self.settings['format'] == 'h5':
            ...
            if self.settings['save_aero']:
                self.ClassesToSave += (sharpy_control.aero.models.aerogrid.Aerogrid,
                                       sharpy_control.utils.datastructures.AeroTimeStepInfo,)
            ...
            if self.settings['save_struct']:
                self.ClassesToSave += (
                    sharpy_control.structure.models.beam.Beam,
                    sharpy_control.utils.datastructures.StructTimeStepInfo,)

            if self.settings['save_linear']:
                self.ClassesToSave += (sharpy_control.solvers.linearassembler.Linear,
                                       sharpy_control.linear.assembler.linearaeroelastic.LinearAeroelastic,
                                       sharpy_control.linear.assembler.linearbeam.LinearBeam,
                                       sharpy_control.linear.assembler.linearuvlm.LinearUVLM,
                                       sharpy_control.linear.src.libss.StateSpace,
                                       sharpy_control.linear.src.lingebm.FlexDynamic,)

            if self.settings['save_linear_uvlm']:
                self.ClassesToSave += (sharpy_control.solvers.linearassembler.Linear, sharpy_control.linear.src.libss.ss_block)
        self.caller = caller
    ...
    def save_timestep(data, settings, ts, hdfile):
        if settings['save_aero']:
            h5utils.add_as_grp(data.aero.timestep_info[ts],
                               ...
                               grpname=("%05d" % ts),
                               ClassesToSave=(sharpy_control.utils.datastructures.AeroTimeStepInfo,),
        ...
        if settings['save_struct']:
            ...
            h5utils.add_as_grp(tstep,
                               ...
                               grpname=("%05d" % ts),
                               ClassesToSave=(sharpy_control.utils.datastructures.StructTimeStepInfo,),
                               SkipAttr=settings['skip_attr'],
                               compress_float=settings['compress_float'])
