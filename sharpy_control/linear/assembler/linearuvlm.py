...
import sharpy_control.linear.utils.ss_interface as ss_interface
import numpy as np
...
import sharpy_control.utils.rom_interface as rom_interface
import sharpy_control.linear.src.libss as libss
from sharpy_control.utils.constants import vortex_radius_def
from sharpy_control.linear.utils.ss_interface import VectorVariable, LinearVector, StateVariable
...
class LinearUVLM(ss_interface.BaseElement):
    ...
    def initialise(self, data, custom_settings=None):
        ...
        if data.aero.n_control_surfaces >= 1:
            import sharpy_control.linear.assembler.lincontrolsurfacedeflector as lincontrolsurfacedeflector
        ...
        if 'u_gust' not in self.settings['remove_inputs'] and self.settings['gust_assembler'] != '':
            import sharpy_control.linear.assembler.lineargustassembler as lineargust
